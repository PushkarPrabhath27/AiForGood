...
import { GuardianEngagementDrawer } from "@/components/guardian/GuardianEngagementDrawer";
...
interface GuardianClientProps {
  id: string;
}

export function GuardianClient({ id }: GuardianClientProps) {
  ...
  const handleNodeClick = (g: Guardian) => {
    setSelectedGuardian(g);
    if (g.engagement_trend && g.engagement_trend !== "stable") {
  ...
  return (
    <div className="space-y-6">
      ...
      <GuardianCard
        guardian={showEngagement ? null : selectedGuardian}
        patientName={patient.name}
        patientId={patient.id}
        open={!!selectedGuardian && !showEngagement}
        onClose={() => setSelectedGuardian(null)}
      />

      {/* Re-engagement Audit Drawer */}
      <GuardianEngagementDrawer
        guardian={showEngagement ? selectedGuardian : null}
        patientName={patient.name}
        open={!!selectedGuardian && showEngagement}
        onClose={() => {
